...
const pointyOrient = hexOrientation(Math.sqrt(3.0), Math.sqrt(3.0) / 2.0, 0.0, 3.0 / 2.0, Math.sqrt(3.0) / 3.0, -1.0 / 3.0, 0.0, 2.0 / 3.0, -0.5);
const flatOrient = hexOrientation(3.0 / 2.0, 0.0, Math.sqrt(3.0) / 2.0, Math.sqrt(3.0), 2.0 / 3.0, 0.0, -1.0 / 3.0, Math.sqrt(3.0) / 3.0, 0.0);
...
function hexIsEquals(hexa, hexb) {
  if (!isHex(hexa) && !isHex(hexb)) throw "Input is not a valid hex"
  return (hexa.q == hexb.q && hexa.r == hexb.r && hexa.s == hexb.s);
}

//coor arithmetic
function hexAdd(a, b) {
  if (!isHex(a) && !isHex(b)) throw "Input is not a valid hex"
...
function hexMult(a, k) {
  if (!isHex(a)) throw "Input is not a valid hex"
...
function hexGetRotate(hex, rotateNum = 1, origin = Hex(0, 0, 0)) {
  var tempHex = hex;
  var hq, hr, hs, tempCoord;
  var negate = 1;
  var coord = hexGetCoord(tempHex);
  var oriCoord = hexGetCoord(origin);
  ...
  switch (rotateNum) {
    ...
    case 1:
      ...
      // return hexMult(hexAdd(Hex(hq,hs,hr),origin),-1);
    case 2:
      tempCoord = hq;
      hq = hs;
      hs = hr;
...
function hexArea(hex, radius, layout) {
  var ctr = Hex(0, 0, 0);
  var boardRadius = layout.radius;
  var result = [];
  ...
  for (var q = -radius + offq; q <= radius + offq; q++) {
    ...
    for (var r = r1; r <= r2; r++) {
      h = Hex(q, -q - r, r);
      if (hexInRadius(h, ctr, boardRadius)) result.push(h);
    }
  }
  return result;
}
...
//becoems hexInRadius
function hexInRadius(hex, ctr, layout) {
  return (includesHex(hexSub(hex, ctr), layout.radius));
}
...
function hexLayout(orientaion, size, radius) //orientaion, point, integer
{
  return {
...
//becomes hexDraw
function hexDraw(layout, hex, color) //layout, hex
{
  var corners = hexGetCorners(layout, hex);
  if (color == "NOFILL") noFill();
  else fill(color);
  beginShape();
  for (var i = 0; i < 6; i++) {
    var pt = corners[i];
