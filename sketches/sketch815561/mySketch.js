...
function init() {
  ...
  let w = (ww + offset * 2 - margin * (cols - 1)) / cols;
  let h = (ww + offset * 2 - margin * (rows - 1)) / rows;
  ...
  for (let k = 0; k < rows; k++) {
    ...
    for (let j = 0; j < cols; j++) {
      let x = -ww / 2 - offset + j * (w + margin);
      let y = -ww / 2 - offset + k * (h + margin);
      movers[k][j] = [];
      for (let i = 0; i < 9; i++) {
        if (random(100) < 33) {
...
class Mover {
  ...
  display() {
    ...
    endShape();
    fill(0,0,100,50);
    ellipse(this.pos.x,this.pos.y,this.bound.w/50,this.bound.w/50);
  }
}
