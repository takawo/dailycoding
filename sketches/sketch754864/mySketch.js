...
function setup() {
  ...
  for (let i = -width / 2; i < width / 2; i += s) {
    for (let j = -height / 2; j < height / 2; j += s) {
      ...
      recursiveTriangle(arr, int(random(0,5)));
      pop();
    }
  }
  count++;
...
function recursiveTriangle(arr, depth) {
  if (depth < 0) {
    return;
  }
  fill(random(pallete));
  ...
  for (let i = 0; i < arr.length; i++) {
    ...
    let p3 = arr[(i + 2) % arr.length];
    let pc2 = p5.Vector.lerp(p1, p2, 0.5);
    let pc3 = p5.Vector.lerp(p1, p3, 0.5);
    let points = [p1, pc2, pc3];
    recursiveTriangle(points, depth - 1);
  }
  let pointsArr = [];
  for (let i = 0; i < arr.length; i++) {
    let p = arr[i];
    let q = arr[(i + 1) % arr.length];
    pointsArr.push(p5.Vector.lerp(p, q, 0.5));
  }
  recursiveTriangle(pointsArr, depth - 1);
}