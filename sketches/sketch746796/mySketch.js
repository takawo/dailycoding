...
let pallete = ["#B0ADB9", "#0F0E18", "#E2E2E6", "#1B3BE7", "#2A4741", "#FBB60A", "#EB36EC", "#228686"];
let bg;
...
function setup() {
  ...
  for (let j = 0; j < rows; j++) {
    for (let i = 0; i < cols; i++) {
      ...
      drawShapesInCircle(0, 0, cellW * 0.9, int(random(5, 12)), pallete);
      pop();
    }
  }
  image(graphics, 0, 0);
...
function drawShapesInCircle(_cx, _cy, _d, _num, colors) {
  ...
  for (let i = 0; i < _num; i++) {
    ...
    let amp = random() * cells / 2;
    let freq = int(random(1, 6)) / 2;
    let c = prevC;
    while (c == prevC) {
      c = random(colors);
    }
