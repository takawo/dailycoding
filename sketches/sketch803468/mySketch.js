...
let url = "https://coolors.co/app/264653-2a9d8f-e9c46a-f4a261-e76f51";
let pallete;
let bg;
...
function draw() {
  ...
  offset = width / 10;
  margin = offset / 10;
  let sqw = sqrt(sq(width) + sq(height));

  cellW = (sqw + offset * 2 - margin * (cols - 1)) / cols;
  cellH = (sqw + offset * 2 - margin * (rows - 1)) / rows;
  push();
  translate(width/2,height/2);
  rotate(int(random(8)) * 360/8);
  for (let j = 0; j < rows; j++) {
    for (let i = 0; i < cols; i++) {
      let x = -sqw/2 - offset + i * (cellW + margin);
      let y = -sqw/2 - offset + j * (cellH + margin);
